namespace _10.Sneaking
{
    ...
    class StartUp
    {
        ...
        static void Main(string[] args)
        {
            ...
            samPosition = GetSamsPosition(samPosition);
            ProccessMoves(moves, samPosition, room);
        }

        private static void ProccessMoves(char[] moves, int[] samPosition, char[][] room)
        {
            for (int i = 0; i < moves.Length; i++)
            {
                for (int row = 0; row < room.Length; row++)
                {
                    ProccessEnemyMoves(room, row);
                }

                int[] getEnemy = new int[2];
                getEnemy = GetEnemyPosition(samPosition, getEnemy);

                CheckIsSamDie(samPosition, room, getEnemy);
        ...
        private static void CheckIsSamDie(int[] samPosition, char[][] room, int[] getEnemy)
        {
        ...
        private static void ProccessEnemyMoves(char[][] room, int row)
        {
            for (int col = 0; col < room[row].Length; col++)
            {
                if (room[row][col] == 'b')
        ...
        private static int[] GetEnemyPosition(int[] samPosition, int[] getEnemy)
        {
            for (int j = 0; j < room[samPosition[0]].Length; j++)
            {
                if (room[samPosition[0]][j] != '.' && room[samPosition[0]][j] != 'S')
        ...
        private static int[] GetSamsPosition(int[] samPosition)
        {
            for (int row = 0; row < room.Length; row++)
            {
                for (int col = 0; col < room[row].Length; col++)
