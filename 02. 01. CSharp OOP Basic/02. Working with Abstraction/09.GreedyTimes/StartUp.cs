namespace _09.GreedyTimes
{
    ...
    class StartUp
    {
        static void Main(string[] args)
        {
            ...
            foreach (var resource in bag)
            {
                Console.WriteLine($"<{resource.Key}> ${resource.Value.Values.Sum()}");

                foreach (var item in resource.Value.OrderByDescending(y => y.Key).ThenBy(y => y.Value))
                {
