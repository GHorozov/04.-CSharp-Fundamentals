...
namespace CosmosX.Entities.Containers
{
    public class ModuleContainer : IContainer
    {
        ...
        public long TotalEnergyOutput => this.energyModules.Values.Select(m => m.EnergyOutput).Sum();

        public long TotalHeatAbsorbing => this.absorbingModules.Values.Select(m => m.HeatAbsorbing).Sum();
